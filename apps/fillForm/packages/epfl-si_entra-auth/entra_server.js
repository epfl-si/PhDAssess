...
const getTokens = async (query) => {
  ...
  const request = await OAuth._fetch(`${ config.getTokenBaseURL }/${config.tenantId}/${ config.getTokenAfterTenantURL }`, 'POST', {
    headers: {
...
const refreshAccessToken = async (props) => {
  ...
  const request = await OAuth._fetch(
    `${ config.refreshTokenBaseURL }/${ config.tenantId }/${ config.refreshTokenAfterTenantURL }`,
    'POST',
    {
      headers: {
        Accept: 'application/json',
